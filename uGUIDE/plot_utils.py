...
def plot_posterior_distribution(samples,
                                config,
                                postprocessing=True,
                                ground_truth=None,
                                idx=0,
                                fig_file=f'posterior_distribution'):
    ...
    if config['size_theta'] != samples.shape[1]:
        ...
        if (postprocessing == True) & (len(config['prior_postprocessing'])
                                       != samples.shape[1]):
            raise ValueError('Theta size set in config does not match theta ' \
                            'size used for training')

    if (ground_truth is not None) and (ground_truth.shape[0]
                                       != config['size_theta']):
        if (postprocessing == True) & (len(config['prior_postprocessing'])
                                       != samples.shape[1]):
            raise ValueError('Ground truth size does not match theta size set '
                             'in config')
    ...
    # if samples are tensors, convert to numpy arrays
    ...
    for p, param in enumerate(prior.keys()):
        ...
        hist_uGUIDE, bin_edges = np.histogram(samples[:, p],
                                              density=False,
                                              bins=50)
        n = len(hist_uGUIDE)
